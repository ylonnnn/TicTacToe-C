...
TicTacToe game_create(int size, Player *players[2]) {
    TicTacToe game =
        (TicTacToe){.size = size, .players = {players[0], players[1]}, .available_moves = size * size};
...
array game_get_optimal_orientation(TicTacToe *game, int turn) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (i < vertical.size) {
             optimal_fn(game, &orientation, &req_moves, array_get(&vertical, i),
                    turn);
        }

        if (i < diagonal.size) {
             optimal_fn(game, &orientation, &req_moves, array_get(&diagonal, i),
                    turn);
        }
    }
